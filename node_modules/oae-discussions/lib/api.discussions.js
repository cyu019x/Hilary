...
var DISCUSSION_UPDATE_FIELDS = ['displayName', 'description', 'visibility','voteCount','replyCount','viewCount'];
...
var updateDiscussion = module.exports.updateDiscussion = function(ctx, discussionId, profileFields, callback) {
    ...
    _.each(profileFields, function(value, field) {
        ...
        } else if (field === 'voteCount') {
            validator.check(value, {'code': 400, 'msg': 'A up vote count cannot be empty'}).notEmpty();
        } else if (field === 'replyCount') {
            validator.check(value, {'code': 400, 'msg': 'A reply count cannot be empty'}).notEmpty();
        } else if (field === 'viewCount') {
            validator.check(value, {'code': 400, 'msg': 'A view count cannot be empty'}).notEmpty();
        }
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        DiscussionsAPI.Authz.canManageDiscussion(ctx, discussion, function(err, canManage) {
            ...
            _getAllMemberIds(discussion.id, function(err, memberIds) {
                ...
                // call function in dao.js to modify canssandra
