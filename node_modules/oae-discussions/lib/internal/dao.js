...
var createDiscussion = module.exports.createDiscussion = function(createdBy, displayName, description, visibility, opts, callback) {
    ...
    //make a new cassandra query, constructUpsertCQL(cf, rowKey, rowValue, values, consistency, ttl)
    //quorum for consistency, ttl for lifespan
...
var updateDiscussion = module.exports.updateDiscussion = function(discussion, profileFields, callback) {
    ...
    //if vote, reply or view count is the only thing updated, skip updating lastModified
    _.each( profileFields, function(key, value){
        if (value=='voteCount' || value=='replyCount' || value=='viewCount'){
            return true; //continue
        }
        storageHash.lastModified = storageHash.lastModified || Date.now(); //if any other field is altered
    });
